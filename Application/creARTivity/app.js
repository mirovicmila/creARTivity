    ...
    .then(function (result) {
      ...
      result.records.forEach(function (record) {
        ...
        nizMuzeja.push({
          ...
          video: record._fields[0].properties.video
        });
      });
      session.run("MATCH (n: Piece) RETURN n").then(function (result2) {
        ...
        session.run("MATCH (e: Exhibit) RETURN e").then(function (result3) {
          var nizIzlozbi = [];
  ...
  session
    ...
    .then(function (result) {
      ...
      session.run("MATCH (n: Piece) RETURN n").then(function (result2) {
        ...
        session.run("MATCH (n: Exhibit) RETURN n").then(function (result3) {
          var nizIzlozbi = [];
          result3.records.forEach(function (record) {
            nizIzlozbi.push({
              //id: record._fields[0].identity.low,
...
app.post("/museum/add", function (req, res) {
  ...
  session
    .run(
      "CREATE (n:Museum {id: $idParam, name: $nameParam, ticketPrice: $ticketPriceParam, type: $typeParam, webLocation: $webLocationParam, workingHours: $workingHoursParam, image: $imageParam, address: $addressParam, video: $videoParam}) RETURN n",
      {
        idParam: id,
        nameParam: name,
...
app.post("/museum/edit", function (req, res) {
  ...
  session
    .run(
      "MATCH (n:Museum) WHERE n.id=$idParam SET n.name=$nameParam, n.ticketPrice=$ticketPriceParam, n.type=$typeParam, n.webLocation= $webLocationParam, n.workingHours=$workingHoursParam, n.image=$imageParam, n.address=$addressParam, n.video=$videoParam RETURN n",
      {
        idParam: id,
        nameParam: name,
        ticketPriceParam: ticketPrice,
...
app.get("/museum/:id", function (req, res) {
  ...
  session
    .run(
      "MATCH (m:Museum) WHERE m.id= $idParam RETURN m.name as name, m.type as type, m.ticketPrice as ticketPrice, m.workingHours as workingHours, m.webLocation as webLocation, m.image as image, m.address as address, m.video as video",
      { idParam: id }
    )
    .then(function (result) {
      ...
      var video = result.records[0].get("video");
      console.log(image);
      ...
      session
        ...
        .then(function (result2) {
          ...
          session
            ...
            session
            .run(
              "OPTIONAL MATCH (e: Exhibit) - [pripada: IS_CURRENTLY_DISPLAYED_IN]-> (m:Museum) WHERE m.id= $idParam return e",
              { idParam: id }
            )
...
//delete museum
app.delete("/museum/:id", function(req,res){
  var id=req.params.id;
  console.log("delete: ",id);
  session
   .run(
     "MATCH (m:Museum) WHERE m.id= $idParam DETACH DELETE m",
     {
       idParam:id,
     }
   )
...
app.delete("/piece/:id", function(req,res){
   ...
   session
    .run(
      "MATCH (p:Piece) WHERE p.id= $idParam DETACH DELETE p",
      {
        idParam:id,
      }
    )
...
app.get("/exhibit/:id", function (req, res) {
  ...
  session
    .run(
      "MATCH (e:Exhibit) WHERE e.id=$idParam RETURN e.name as name, e.duration as duration, e.description as description, e.image as image",
      { idParam: id }
    )
    .then(function (result) {
      //console.log(result);
...
app.post("/exhibit/add", function (req, res) {
  ...
  session
    .run(
      "CREATE (e:Exhibit {id: $idParam, name: $nameParam, duration: $durationParam, description: $descriptionParam, image: $imageParam}) RETURN e",
      {
        idParam: id,
        nameParam: name,
...
//edit exhibita
app.post("/exhibit/edit", function (req, res) {
  ...
  session
    .run(
      "MATCH (e:Exhibit) WHERE e.id=$idParam SET e.name= $nameParam, e.duration= $durationParam, e.description=$decriptionParam, e.image=$imageParam ",
      {
        idParam: id,
        nameParam: name,
        durationParam: duration,
        decriptionParam:description,
        imageParam: image
      }
    )
    .then(function (result) {
...
//delete exhibit-a
app.delete("/exhibit/:id", function(req,res){
  var id=req.params.id;
  console.log("delete: ",id);
  session
   .run(
     "MATCH (e:Exhibit) WHERE e.id= $idParam DETACH DELETE e",
     {
       idParam:id,
     }
   )
