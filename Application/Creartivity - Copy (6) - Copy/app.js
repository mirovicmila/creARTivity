...
app.post("/museum/edit", function (req, res) {
  ...
  session
    .run(
      "MATCH (n:Museum {name: $nameParam}) SET n.ticketPrice=$ticketPriceParam, n.type=$typeParam, n.webLocation= $webLocationParam, n.workingHours=$workingHoursParam, n.image=$imageParam, n.address=$addressParam RETURN n",
      {
        nameParam: name,
        ticketPriceParam: ticketPrice,
        typeParam: type,
...
app.delete("/museum/:id", function(req, res){
  res.send("Makar poziva delete request");
})
//vraca muzej
...
app.post("/piece/edit", function (req, res) {
  ...
  session
    .run(
      "MATCH (p:Piece {name: $nameParam}) SET (p.type= $typeParam, p.author=$authorParam, p.description=$descriptionParam, p.link=$linkParam, p.image=$imageParam) ",
      {
        nameParam: name,
        typeParam: type,
        authorParam: author,
...
//vraca izlozbu
app.get("/exhibition/:id", function (req, res) {
  var id = req.params.id;
  session
    .run(
      "MATCH (e:Exhibition) WHERE id(e)=toInteger($idParam) RETURN e.name as name, e.duration as duration, e.description as description, e.image as image",
      { idParam: id }
    )
    .then(function (result) {
      //console.log(result);
